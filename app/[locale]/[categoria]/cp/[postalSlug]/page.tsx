...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = `${catName} en ${postalData.name} | The Games Room`;
  const description = `${catName} en ${postalData.name} (${parsed.postalCode}). Envío gratis, garantía 2 años. Instalación profesional en ${postalData.region}.`;
...
export default async function PostalCodePage({ params }: PageProps) {
  ...
  const t: Record<string, Record<string, string>> = {
    es: { from: 'Desde', quote: 'Presupuesto Gratis', warranty: '2 años garantía', quality: 'Calidad premium', delivery: 'Entrega rápida', nearby: 'Zonas cercanas', otherProducts: 'Otros productos', viewAll: 'Ver catálogo' },
